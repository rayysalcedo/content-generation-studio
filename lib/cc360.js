...
function backendHeaders(token, locationId, tokenId) {
  const h = {
    ...
    'source': 'WEB_USER',
    'sourceid': locationId,
    // These three are what the browser sends — GHL's backend.* endpoints validate
    // origin/referer for CSRF protection. Without them, valid JWTs still get 401.
    'origin': 'https://app.coursecreator360.com',
    'referer': 'https://app.coursecreator360.com/',
    'user-agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/148.0.0.0 Safari/537.36',
  };
  // Firebase ID token — backend.* validates this alongside the User JWT.
  // Without it, requests 401 even with a valid User JWT.
