...
// =====================================================================
// Membership API (services.leadconnectorhq.com/membership/...)
// CC360's UI calls these from an iframe hosted at backend.memberships.apisystem.tech,
// and Cloudflare WAF allowlists by Origin header. From the server we spoof the same
// Origin/Referer (browsers can't but Node can), which lets the WAF accept the call.
...
function backendHeaders(token, locationId, tokenId) {
  // Origin/Referer must be the iframe origin (backend.memberships.apisystem.tech),
  // NOT app.coursecreator360.com. Cloudflare WAF allowlists by Origin — using the
  // CC360 page origin gets 401, the iframe origin sails through.
  // Confirmed exact-match with a working "Copy as cURL" capture from CC360's UI.
  const h = {
...
export async function fetchCourseStructure({ token, tokenId, locationId, productId }) {
  const url = `https://services.leadconnectorhq.com/membership/locations/${encodeURIComponent(locationId)}/categories?product_id=${encodeURIComponent(productId)}&posts=true`;
...
export async function updateCourseThumbnail({ token, tokenId, locationId, productId, posterImageUrl, title, description }) {
  const url = `https://services.leadconnectorhq.com/membership/locations/${encodeURIComponent(locationId)}/products/${encodeURIComponent(productId)}`;
...
export async function updateLessonThumbnail({ token, tokenId, locationId, post, posterImageUrl }) {
  ...
  const url = `https://services.leadconnectorhq.com/membership/locations/${encodeURIComponent(locationId)}/posts/${encodeURIComponent(post.id)}`;
...
export async function attachThumbnails({
  ...
  if (!structure && (wantsCourseThumb || wantsAnyLessonThumb)) {
    throw new Error(
      authRejected
        ? `services.leadconnectorhq.com rejected auth: ${lastPollError}`
