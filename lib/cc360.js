...
// =====================================================================
// Internal Backend API (backend.memberships.apisystem.tech)
// Note: this host CORS-blocks browser calls (intentional — server-to-server only).
// Must be called from the Node server with a User-authClass JWT.
...
export async function fetchCourseStructure({ token, tokenId, locationId, productId }) {
  const url = `https://backend.memberships.apisystem.tech/membership/locations/${encodeURIComponent(locationId)}/categories?product_id=${encodeURIComponent(productId)}&posts=true`;
...
export async function updateCourseThumbnail({ token, tokenId, locationId, productId, posterImageUrl, title, description }) {
  const url = `https://backend.memberships.apisystem.tech/membership/locations/${encodeURIComponent(locationId)}/products/${encodeURIComponent(productId)}`;
...
export async function updateLessonThumbnail({ token, tokenId, locationId, post, posterImageUrl }) {
  ...
  const url = `https://backend.memberships.apisystem.tech/membership/locations/${encodeURIComponent(locationId)}/posts/${encodeURIComponent(post.id)}`;
...
export async function attachThumbnails({
  ...
  if (!structure && (wantsCourseThumb || wantsAnyLessonThumb)) {
    throw new Error(
      authRejected
        ? `backend.memberships.apisystem.tech rejected auth: ${lastPollError}`
