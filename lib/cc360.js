...
/**
 * Build the import payload. Note: lesson thumbnails are NO LONGER embedded
 * in the lesson HTML body — they're attached via attachThumbnails() after
 * import, into the proper Lesson Thumbnail field.
 */
export function buildImportPayload({ locationId, draft, accent, workbookUrlByLessonKey = {} }) {
...
export async function importCourse({ pit, locationId, draft, accent, workbookUrlByLessonKey }) {
  const payload = buildImportPayload({ locationId, draft, accent, workbookUrlByLessonKey });
...
export async function attachThumbnails({
  ...
  while (Date.now() - startTime < maxWaitMs) {
    try {
      const data = await fetchCourseStructure({ token, locationId, productId });
      const categories = data?.categories || [];
      const totalPosts = categories.reduce((n, c) => n + (c.posts?.length || 0), 0);
      // If we expected lessons, wait until they're all there. Otherwise as soon as any data shows up.
      const expectedTotalPosts = wantsAnyLessonThumb
        ? Math.max(...expectedLessonKeys.map(k => {
            const m = parseInt(k.match(/m(\d+)/)?.[1] || '0', 10);
            const l = parseInt(k.match(/l(\d+)/)?.[1] || '0', 10);
            return (m * 1000) + l + 1;     // crude max-position; replaced below
          }), 0)
        : 0;
      // Cleaner: expectedTotalPosts = number of unique keys
      ...
      // Course not ready yet; keep polling
      onProgress({ phase: 'polling-retry', error: e.message });
    }
    await new Promise(r => setTimeout(r, pollIntervalMs));
  }

  if (!structure && (wantsCourseThumb || wantsAnyLessonThumb)) {
    throw new Error(`Course structure did not appear within ${maxWaitMs / 1000}s — could not attach thumbnails`);
  }
