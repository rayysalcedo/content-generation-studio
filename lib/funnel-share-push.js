...
//   Funnel" path in the platform's UI uses /funnels/funnel/clone-funnel-to-locations
...
//   Bearer), not by app.gohighlevel.com (RS256 Bearer). We try with
...
//
// WAF / Origin:
//   Origin: https://app.gohighlevel.com (always the master domain, not a
//   white-label one — this endpoint was captured from the GHL master UI; white-label users
//   reach it via the share-link click flow which redirects to gohighlevel.com).
//   Same trick as lib/ghl.js — Node spoofs Origin since the WAF gates by
//   header, not IP.
// ───────────────────────────────────────────────────────────────────
...
// Headers verified via Network-tab HAR capture of GoHighLevel "Share Funnel
// → Import" UI flow. Differs from snapshot-push.js in four places:
//   • origin: app.gohighlevel.com (master domain, not a white-label one)
//   • app-name: spm-ts (sales-pipeline-management micro-app)
//   • x-translations-lang: en-US
//   • NO source-id header (snapshot endpoint required `source-id: locations`)
// Note: `version: 2021-07-28` is REQUIRED for v2 API (without it, 401
// "version header was not found"). Chrome's "Copy as cURL" sometimes
// drops this header in its output — the HAR confirms it's actually sent.
...
export async function cloneFunnelToLocations({
  ...
  if (res.status < 200 || res.status >= 300) {
    ...
    let hint = '';
    if (res.status === 401) {
      hint = ' (Bearer JWT rejected — the share endpoint may require a JWT issued by app.gohighlevel.com rather than a white-label domain. Capture a fresh JWT from any logged-in app.gohighlevel.com tab and paste it at /setup.)';
      ...
      hint = ' (The funnel ID may not exist or may not be accessible to this user. Verify the funnelId by opening the funnel in GoHighLevel and checking its URL.)';
    }
