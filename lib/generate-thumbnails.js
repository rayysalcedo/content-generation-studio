...
// using Gemini Nano Banana (gemini-3.1-flash-image by default).
//
...
//
// Gemini image models need a Gemini API key with billing enabled for
// production volume; the free tier has low image quotas.
//
...
const DEFAULT_MODEL = 'gemini-3.1-flash-image';  // Nano Banana 2. Cheaper: gemini-3.1-flash-lite-image (1K only). Best: gemini-3-pro-image
const DEFAULT_IMAGE_SIZE = '1K';               // '512' | '1K' | '2K' | '4K'

// Concurrency — Gemini image rate limits depend on your billing tier. Start
// conservative (3) and raise via env once you know your quota:
//   AI_IMAGE_CONCURRENCY=8  AI_IMAGE_DELAY_MS=200
const BATCH_SIZE     = Number(process.env.AI_IMAGE_CONCURRENCY) || 3;
const BATCH_DELAY_MS = Number(process.env.AI_IMAGE_DELAY_MS)    || 500;

// Gemini aspect ratios for each slot
const SIZE_LANDSCAPE = '3:2';        // for course hero
const SIZE_SQUARE    = '1:1';        // for lesson icons
 ...
 * @param {string} [opts.model]          - override image model (defaults to gemini-3.1-flash-image)
 * @param {string} [opts.imageSize]      - '512' | '1K' | '2K' | '4K' (default '1K')
...
export async function generateThumbnailsForCourse({
  ...
  for (let i = 0; i < queue.length; i += BATCH_SIZE) {
    ...
    const results = await Promise.all(
      batch.map(item => generateOneImage(apiKey, item, { model, imageSize }).catch(err => {
...
// ---------------------------------------------------------------------
// Prompt builders — natural-language prompts; Gemini follows detailed style directions well
// ---------------------------------------------------------------------
...
// ---------------------------------------------------------------------
// Single image generation — calls Gemini, returns base64 PNG
// ---------------------------------------------------------------------
async function generateOneImage(apiKey, { prompt, label, size }, { model, imageSize }) {
  return await retryAi(async () => {
    return await generateImageBase64({ apiKey, model, prompt, aspectRatio: size, imageSize });
  }, {
    ...
    shouldRetry: (err) => {
      ...
      // 429 (rate limit / quota) and 5xx are transient; safety refusals are not
      if (msg.includes('rate limit') || msg.includes('429') || msg.includes('resource_exhausted')) return true;
      if (msg.includes('no image data')) return true;
      if (msg.includes('content policy') || msg.includes('safety') || msg.includes('blocked') || msg.includes('prohibited')) return false;
      return false;
    },
  });
}
...
// ---------------------------------------------------------------------
// Helper: convert a base64 string into a Buffer (for upload to GoHighLevel)
// ---------------------------------------------------------------------
