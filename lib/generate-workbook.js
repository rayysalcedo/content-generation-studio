...
// for each lesson in a course structure, using OpenAI's Chat Completions API in JSON mode.
//
import OpenAI from 'openai';
import { retryAi, isTransient } from './retry.js';

// OpenAI handles concurrency well, so we can parallelize more aggressively
// than the old Gemini setup which was serial-with-6s-delay
const BATCH_SIZE = 3;          // 3 lessons at a time
...
export async function generateWorkbooksForCourse({
  ...
  for (let i = 0; i < queue.length; i += BATCH_SIZE) {
    ...
    const results = await Promise.all(
      batch.map(item => generateOneWorkbook(openai, item, model).catch(err => {
...
async function generateOneWorkbook(openai, { lesson, courseTitle, moduleTitle }, model) {
