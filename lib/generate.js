...
// using OpenAI's Chat Completions API in JSON mode.
//
import OpenAI from 'openai';
import { retryAi } from './retry.js';

// Safety ceiling on PDF text sent to the model. Modern OpenAI models support
...
export async function generateCourseStructure({
  ...
  return await retryAi(async () => {
    const completion = await openai.chat.completions.create({
      ...
      temperature: 0.4,
      // Bumped from 16000 → 24000 to make room for larger courses (9+ modules
      // with rich lesson bodies can brush the 16k ceiling on the output side).
      max_tokens: 24000,
    });
