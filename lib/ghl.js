// lib/ghl.js — Push a course draft to GoHighLevel + attach thumbnails to proper fields.
//
// Two-phase push:
...
//   2. attachThumbnails() → use the platform's internal backend API to set posterImage on
 ...
 * the import endpoint attaches thumbnails during course creation. If GoHighLevel honors these
 ...
 * fallback (requires GHL_USER_JWT for backend.* auth).
 */
    ...
    products: [
      {
        ...
        // Explicitly publish the course — without this, GHL defaults to "draft" and the
        // course's lessons never appear in GET /categories (the polling we do for attach).
        ...
        categories: (draft.modules || []).map((mod, mi) => ({
          ...
          posts: (mod.lessons || []).map((lesson, li) => {
            ...
            return {
              ...
              contentType: 'video',
              // Same here — without explicit visibility, lessons stay in draft and
              // are invisible to /categories (which is how the attach polls find them).
              visibility: 'published',
...
// =====================================================================
// Membership API (backend.leadconnectorhq.com/membership/...)
// Captured the working request from the platform's Network tab → Request Headers.
// Plain cross-origin: app.gohighlevel.com → backend.leadconnectorhq.com.
// Just needs the right User JWT + standard headers + `version: 2021-07-28`.
...
function backendHeaders(token, locationId, tokenId) {
  // Headers verified by capturing the actual Request Headers panel from a working
  // GoHighLevel UI call (Network tab → Headers → Request Headers section). The page is at
  // app.gohighlevel.com and makes plain cross-origin calls to backend.leadconnectorhq.com.
  // No iframe-origin spoofing needed; the `version: 2021-07-28` header was what we'd
  // been missing — it's GHL's API versioning header.
  const h = {
 ...
 * We need the actual postIds to attach thumbnails — they're assigned by GoHighLevel
 * after import, not by us.
 *
 * The API returns a top-level JSON ARRAY (not an object with `categories`).
 * We normalize the shape to `{ categories: [...] }` so the rest of the code
 * (and any callers) can use a stable structure.
 */
export async function fetchCourseStructure({ token, tokenId, locationId, productId }) {
  ...
  // GHL's API returns the categories array at the top level (proven by a Network
  // capture of a working call). Older docs/responses sometimes wrapped it in
  // { categories: [...] } — handle both shapes defensively.
...
export async function updateLessonThumbnail({ token, tokenId, locationId, post, posterImageUrl }) {
  ...
  // Mirror the field list the GoHighLevel web UI sends. Defaults fill in anything missing
...
export async function attachThumbnails({
  ...
  maxWaitMs = 60_000, pollIntervalMs = 1_500,   // Lessons usually visible immediately; 60s ceiling is safety net
  onProgress = () => {},
}) {
  ...
  while (Date.now() - startTime < maxWaitMs) {
    ...
    try {
      ...
      // Log every 5th attempt (~10s) so progress is visible without spamming
      if (pollAttempts === 1 || pollAttempts % 5 === 0) {
        onProgress({ phase: 'polling', totalPosts, expected: expectedLessonKeys.length, elapsedMs: Date.now() - startTime });
      }
    } catch (e) {
  ...
  if (!structure && (wantsCourseThumb || wantsAnyLessonThumb)) {
    ...
    throw new Error(
      authRejected
        ...
        : `Course structure did not appear within ${elapsed}s (${pollAttempts} attempts). Last seen: ${lastTotalPosts >= 0 ? `${lastTotalPosts}/${expectedLessonKeys.length} lessons ready` : 'no successful poll'}. ${lastPollError ? `Last error: ${lastPollError}` : ''}`
    );
  }
