...
export function renderLessonHTML(lesson, theme, opts = {}) {
  ...
  const { workbookUrl, thumbnailUrl } = opts;   // optional: workbook PDF URL + lesson thumbnail URL

  const thumbnailBlock = thumbnailUrl
    ? `<div style="margin:0 0 28px 0;text-align:center;">
         <img src="${escapeAttr(thumbnailUrl)}" alt="" style="max-width:100%;height:auto;border-radius:12px;display:inline-block;box-shadow:0 4px 16px rgba(0,0,0,0.08);" />
       </div>`
    : '';
...
${thumbnailBlock}${summaryBlock}${takeawaysBlock}${sectionsBlock}${proTipBlock}${reflectionBlock}${workbookBlock}
</div>`;
}
