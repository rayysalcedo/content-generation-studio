...
// Loads a CC360/GHL agency snapshot into a target sub-account using
...
//   UI action. The CC360 web UI calls a private endpoint on
...
//   it wants, so we pass `origin: https://app.coursecreator360.com`
//   and the WAF lets us through. This is the same trick used by the
//   thumbnail-attach flow in lib/cc360.js — verified working in prod.
//
// Auth pair (NOT OAuth):
//   • authorization: Bearer <User JWT>   — the agency-admin's user-session JWT
//   • token-id: <Firebase ID token>       — CSRF-style companion header
//   Both are captured by the CC360-side Custom JS snippet and stored on
...
// Headers verified via Network-tab capture of CC360's "Load Snapshot" UI.
// Notably DIFFERENT from membership-API headers in lib/cc360.js:
//   • `source-id: locations` (literal string, NOT a locationId)
//   • no `version` header (the snapshots-appengine endpoint doesn't take one)
...
export async function fetchSnapshotAssets({
  ...
  if (!userJwt)    throw new Error('fetchSnapshotAssets: missing userJwt (paste at /setup or have CC360 tab open)');
...
export async function loadSnapshotToLocation({
  ...
  if (!userJwt)    throw new Error('loadSnapshotToLocation: missing userJwt (paste at /setup or have CC360 tab open)');
