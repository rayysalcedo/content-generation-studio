...
if (USE_PG) {
  ...
  `).then(() => {
    ...
    // Do NOT rethrow: a dead DB must not crash-loop the server. Individual
    // store calls will fail with a clear error instead.
    pool = null;
  });
} else {
...
function requirePool() {
  if (!pool) throw new Error('Token store unavailable: Postgres connection failed at boot (check DATABASE_URL)');
}

async function pgGet(locationId) {
  await ready; requirePool();
