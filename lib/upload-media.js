// lib/upload-media.js — Upload files to CC360's media systems.
//
// Two upload paths:
...
export async function uploadCourseMedia({
  ...
  const id = filename || `thumb-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;

  // ---- Step 1: get a signed PUT URL ----
  // NOTE: this endpoint only accepts User-authClass JWTs (the ones the CC360 web app
  ...
  // Try every plausible name and surface the raw shape on miss so we can lock it down.
  const signedUrl =
    raw.signedUrl ||
    raw.uploadUrl ||
    raw.putUrl ||
  ...
  const publicUrl =
    ...
    raw.cdnUrl ||
    raw.assetUrl ||
    // Last-resort: construct from a returned path on the standard CDN
